# -------------------------------------------------------------------------------
# Name:        Enhanced_PEI_2
# Purpose: The purpose of this script is to calculate the second enhanced Pedestrian Environment Index, which combines
# the land use diversity, population density, commercial density and intersection density sub metrics just like with
# the original. It also adds the access to public transportation metric as an enhancer to the PEI. Effectively, this script combines
# the code from the Land Use Mix, Population Density, Commercial Density, Intersection Density, and public transportation access
# scripts.
#
# Steps
...
# Step 5: Calculate Access to Public Transportation metric.
...
def main():
    ...
    transportation_points = arcpy.GetParameterAsText(11)
    # Unique ID of transportation (subway and bus) points.
    transportation_id_field = arcpy.GetParameterAsText(12)
    ...
    # Step 5: Calculate Public Transportation Access metric.
    ...
    # Step 5.3: Summarize selected lines based on transportation point ID and calculate ratio field.
    ...
    with arcpy.da.UpdateCursor(geographical_units, ["ct_id", "land_use_diversity", "commercial_density", "intersection_density", "transportation_access"]) as cursor:
        for row in cursor:
            row[1] = land_use_diversity[row[0]]
            row[2] = commercial_density[row[0]]
            row[3] = intersection_density[row[0]]
            row[4] = transportation_access[row[0]]
            cursor.updateRow(row)
    arcpy.CalculateField_management(geographical_units, output, f"((1 + !land_use_diversity!) * (1 + !pop_density!) * (1 + !commercial_density!) * (1 + !intersection_density!) * (1 + !transportation_access!))/32")
