# -------------------------------------------------------------------------------
# Name:        Enhanced_PEI_4
# Purpose: The purpose of this script is to calculate the fourth enhanced Pedestrian Environment Index, which combines
# the land use diversity, population density, commercial density and intersection density sub metrics just like with
# the original. It also adds the street network density metric as an enhancer to the PEI. Effectively, this script combines
# the code from the Land Use Mix, Population Density, Commercial Density, Intersection Density, and Street Network Density
# scripts.
#
# Steps
...
# Step 5: Calculate Street Network Density metric.
# Step 5.1: Buffer roads using the street width field.
# Step 5.2: Apportion polygons to find street area in square feet per census tract.
# Step 5.3: Calculate unnormalized street network density and max normalization.
...
def main():
    ...
    roads_area_field = arcpy.GetParameterAsText(11)
    # Census tracts feature class.
    ...
    with arcpy.da.UpdateCursor(geographical_units, ["ct_id", "land_use_diversity", "commercial_density", "intersection_density", "sn_density"]) as cursor:
        for row in cursor:
            ...
            cursor.updateRow(row)
    arcpy.CalculateField_management(geographical_units, output, f"((1 + !land_use_diversity!) * (1 + !pop_density!) * (1 + !commercial_density!) * (1 + !intersection_density!) * (1 + !sn_density!))/32")
