...
def main():
    ...
    sidewalks = arcpy.GetParameterAsText(11)
    # Area field of the sidewalk feature class.
    sidewalk_area_field = arcpy.GetParameterAsText(12)
    ...
    arcpy.AddField_management(geographical_units, "PEI_v2", "DOUBLE")

    with arcpy.da.UpdateCursor(geographical_units, ["ct_id", "land_use_diversity", "commercial_density", "intersection_density", "sidewalk_density"]) as cursor:
        for row in cursor:
            row[1] = land_use_diversity[row[0]]
            row[2] = commercial_density[row[0]]
            row[3] = intersection_density[row[0]]
            row[4] = sidewalk_density[row[0]]
            cursor.updateRow(row)
        arcpy.AddField_management(output, "PEI", "DOUBLE")
    arcpy.CalculateField_management(geographical_units, "PEI_v2", f"((1 + !land_use_diversity!) * (1 + !pop_density!) * (1 + !commercial_density!) * (1 + !intersection_density!) * (1 + !sidewalk_density!))/32")
